...
def _fusionner(ligne: List[int]) -> Tuple[List[int], int]:
    ...
    while i < len(ligne):
        if i + 1 < len(ligne) and ligne[i] == ligne[i + 1]:
            ...
            points += fusion
            ligne_fusionnee.append(fusion)
            i += 2
        else:
            ligne_fusionnee.append(ligne[i])
            i += 1
    return ligne_fusionnee, points
